...
namespace SafTRDPClient
{
    ...
    public partial class frm_main : Form
    {
        ...
        private bool RegValueExists(string text,string root,string registry_path)
        {
            ...
            switch (root)
            {
                ...
                case "CC":
                    ...
                    {
                        MessageBox.Show("Couldn't get list of variables for registry key", "Error working with registry");
                        break;
                    }
                    break;
            }
            ...
            {
                MessageBox.Show("Failed to retrieve values", "Error working with registry");
            }
            return exists;
        }
        ...
        public void BuildTree()
        {
            ...
            RegistryKey group = null;
            string[] groups = null;//Список групп
            string[] servers = null;//Список серверов

            string groupstring = "Software\\SafT\\RDPClient\\Groups";
            try
            {
                group = Registry.CurrentUser.OpenSubKey(groupstring);
            }
            catch
            {
                group = null;
                MessageBox.Show(group.ToString(), "group");
            }
            

            if (group != null)
            {
                groups = group.GetValueNames();
            ...
            if (servlist != null)
            {
                servers = servlist.GetSubKeyNames();
            }
            servlist.Close();
        ...
        private void frm_main_Load(object sender, EventArgs e)
        {
            ...
            if (domains != null)
            {
                ...
                {
                    FastConnect_Domain_TxtBx.Items.Add(domainslist[i]);
                }
        ...
        {
            label6.Text = (FastConnect_Password_TxtBx.Text.Length.ToString());
            label7.Text = (FastConnect_Password_TxtBx.Text.GetHashCode().ToString());
...
// добавления сервера на соответствующей форме
        ...
        {
            connected_screens++;
            FastConnect_ConnectBtn.Text = "Disconnect";
        ...
        {
            connected_screens--;
            FastConnect_ConnectBtn.Text = "Connect";
            TreeNode helper = (TreeNode)RDPClient.Tag;
            helper.ImageIndex = 5;
        ...
        {
            RegistryKey folders = null;

            try
            {
                Registry.CurrentUser.OpenSubKey(Common_functions.Settings.common_reg_string + "\\Groups");
            }
            catch
            {
                MessageBox.Show(Common_functions.Settings.common_reg_string + "\\Groups","Ошибка открытия пути реестра");
            }
            if (folders != null)
            {
